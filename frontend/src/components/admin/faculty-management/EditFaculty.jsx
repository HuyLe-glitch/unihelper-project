import React, { useState, useEffect } from 'react';
import { ArrowLeft, Save, Loader } from 'lucide-react';
import './FacultyForm.css';

const EditFaculty = () => {
  // TODO: Get faculty ID from URL params
  // const { id } = useParams();
  const facultyId = '1'; // Mock ID

  const [formData, setFormData] = useState({
    name: '',
    code: '',
    description: '',
    isActive: true
  });
  const [errors, setErrors] = useState({});
  const [isLoading, setIsLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Mock data - Replace with API call
  const mockFaculty = {
    _id: '1',
    name: 'Khoa Công nghệ Thông tin',
    code: 'CNTT',
    description: 'Đào tạo các chuyên ngành về công nghệ thông tin, phần mềm, mạng máy tính',
    isActive: true,
    majorCount: 5
  };

  // Load faculty data
  useEffect(() => {
    const fetchFaculty = async () => {
      try {
        setIsLoading(true);
        
        // TODO: Call API to get faculty by ID
        // const response = await getFacultyById(facultyId);
        
        // Simulate API call
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // Set form data from API response
        setFormData({
          name: mockFaculty.name,
          code: mockFaculty.code,
          description: mockFaculty.description || '',
          isActive: mockFaculty.isActive
        });
        
      } catch (error) {
        console.error('Error fetching faculty:', error);
        alert('Có lỗi xảy ra khi tải thông tin khoa!');
      } finally {
        setIsLoading(false);
      }
    };

    fetchFaculty();
  }, [facultyId]);

  // Validation
  const validateForm = () => {
    const newErrors = {};
    
    if (!formData.name.trim()) {
      newErrors.name = 'Tên khoa là bắt buộc';
    } else if (formData.name.length > 100) {
      newErrors.name = 'Tên khoa không quá 100 ký tự';
    }
    
    if (!formData.code.trim()) {
      newErrors.code = 'Mã khoa là bắt buộc';
    } else if (formData.code.length > 10) {
      newErrors.code = 'Mã khoa không quá 10 ký tự';
    }
    
    if (formData.description && formData.description.length > 500) {
      newErrors.description = 'Mô tả không quá 500 ký tự';
    }

    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };

  // Handlers
  const handleChange = (e) => {
    const { name, value, type, checked } = e.target;
    setFormData(prev => ({
      ...prev,
      [name]: type === 'checkbox' ? checked : value
    }));
    // Clear error when user types
    if (errors[name]) {
      setErrors(prev => ({ ...prev, [name]: '' }));
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    
    if (!validateForm()) {
      return;
    }

    setIsSubmitting(true);

    try {
      // TODO: Call API to update faculty
      // const response = await updateFaculty(facultyId, {
      //   ...formData,
      //   code: formData.code.toUpperCase()
      // });
      
      // Simulate API call
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      console.log('Updating faculty:', {
        id: facultyId,
        ...formData,
        code: formData.code.toUpperCase()
      });

      alert('Cập nhật khoa thành công!');
      
      // TODO: Navigate back to faculty list
      // navigate('/faculty');
      handleCancel();
      
    } catch (error) {
      console.error('Error updating faculty:', error);
      alert('Có lỗi xảy ra khi cập nhật khoa!');
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleCancel = () => {
    // TODO: Navigate back to faculty list
    if (window.confirm('Bạn có chắc chắn muốn hủy? Các thay đổi chưa lưu sẽ bị mất.')) {
      alert('Quay lại trang danh sách khoa');
    }
    navigate(-1);
  };

  // Loading state
  if (isLoading) {
    return (
      <div className="faculty-form-container">
        <div className="faculty-form-wrapper">
          <div className="loading-container">
            <Loader className="loading-spinner" size={48} />
            <p className="loading-text">Đang tải thông tin khoa...</p>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="faculty-form-container">
      <div className="faculty-form-wrapper">
        {/* Header */}
        <div className="form-header">
          <button onClick={handleCancel} className="btn-back">
            <ArrowLeft size={20} />
            Quay lại
          </button>
          <div className="form-header-content">
            <h1 className="form-title">Chỉnh sửa khoa</h1>
            <p className="form-subtitle">Cập nhật thông tin khoa: {mockFaculty.name}</p>
          </div>
        </div>

        {/* Form */}
        <div className="form-content">
          <div className="form-card">
            <div className="form-section">
              <h2 className="section-title">Thông tin cơ bản</h2>
              
              <div className="form-grid">
                <div className="form-group">
                  <label htmlFor="name" className="form-label">
                    Tên khoa <span className="required">*</span>
                  </label>
                  <input
                    type="text"
                    id="name"
                    name="name"
                    value={formData.name}
                    onChange={handleChange}
                    className={`form-input ${errors.name ? 'form-input-error' : ''}`}
                    placeholder="Nhập tên khoa"
                    maxLength={100}
                  />
                  {errors.name && <p className="error-message">{errors.name}</p>}
                </div>

                <div className="form-group">
                  <label htmlFor="code" className="form-label">
                    Mã khoa <span className="required">*</span>
                  </label>
                  <input
                    type="text"
                    id="code"
                    name="code"
                    value={formData.code}
                    onChange={(e) => {
                      handleChange({
                        target: {
                          name: 'code',
                          value: e.target.value.toUpperCase(),
                          type: 'text'
                        }
                      });
                    }}
                    className={`form-input form-input-code ${errors.code ? 'form-input-error' : ''}`}
                    placeholder="VD: CNTT"
                    maxLength={10}
                  />
                  {errors.code && <p className="error-message">{errors.code}</p>}
                  <p className="form-hint">Mã khoa sẽ tự động chuyển thành chữ in hoa</p>
                </div>
              </div>

              <div className="form-group">
                <label htmlFor="description" className="form-label">
                  Mô tả
                </label>
                <textarea
                  id="description"
                  name="description"
                  value={formData.description}
                  onChange={handleChange}
                  className={`form-textarea ${errors.description ? 'form-input-error' : ''}`}
                  placeholder="Nhập mô tả về khoa"
                  rows={5}
                  maxLength={500}
                />
                {errors.description && <p className="error-message">{errors.description}</p>}
                <p className="char-count">
                  {formData.description.length}/500 ký tự
                </p>
              </div>

              <div className="form-group">
                <div className="checkbox-wrapper">
                  <input
                    type="checkbox"
                    id="isActive"
                    name="isActive"
                    checked={formData.isActive}
                    onChange={handleChange}
                    className="form-checkbox"
                  />
                  <label htmlFor="isActive" className="checkbox-label">
                    <span className="checkbox-title">Khoa đang hoạt động</span>
                    <span className="checkbox-description">
                      Khoa sẽ được hiển thị và có thể đăng ký chuyên ngành
                    </span>
                  </label>
                </div>
              </div>
            </div>

            {/* Info Section */}
            <div className="form-section info-section">
              <h2 className="section-title">Thông tin bổ sung</h2>
              <div className="info-grid">
                <div className="info-item">
                  <span className="info-label">Số chuyên ngành:</span>
                  <span className="info-value">{mockFaculty.majorCount} chuyên ngành</span>
                </div>
                <div className="info-item">
                  <span className="info-label">Mã khoa hiện tại:</span>
                  <span className="info-value info-code">{mockFaculty.code}</span>
                </div>
              </div>
              <p className="info-warning">
                ⚠️ Lưu ý: Thay đổi trạng thái khoa sẽ ảnh hưởng đến tất cả các chuyên ngành thuộc khoa này.
              </p>
            </div>
          </div>

          {/* Action Buttons */}
          <div className="form-actions">
            <button
              type="button"
              onClick={handleCancel}
              className="btn-cancel"
              disabled={isSubmitting}
            >
              Hủy
            </button>
            <button
              type="button"
              onClick={handleSubmit}
              className="btn-submit"
              disabled={isSubmitting}
            >
              <Save size={20} />
              {isSubmitting ? 'Đang lưu...' : 'Cập nhật'}
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

export default EditFaculty;
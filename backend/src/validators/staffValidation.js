...
/**
 * Staff Validation - Đơn giản hóa cho hệ thống 2 staff cố định
 * 
 * LƯU Ý: Hệ thống chỉ có 2 tài khoản staff cố định (CTSV & KTX)
 * KHÔNG cần validators cho create/update staff vì không tạo thêm được
 * 
 * Các validators còn lại chỉ dùng cho việc xử lý requests
 */
...
class StaffValidator {
  ...
  /**
   * Validate staff permissions - kiểm tra quyền xử lý request
   */
  validateStaffPermissions = async (req, res, next) => {
    try {
      // Staff CTSV chỉ xử lý CTSV requests
      // Staff KTX chỉ xử lý KTX requests
      next();
    } catch (error) {
      res.status(403).json({
        status: false,
  ...
  /**
   * Validator cho lấy thông tin staff theo type
   * (Hệ thống chỉ có 2 staff cố định: CTSV và KTX)
   */
  getStaffByTypeValidator = [
    param('staffType')
      .notEmpty()
